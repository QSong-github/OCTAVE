...
def fig6():
    ...
    METR = [("ari", m1, "Spatial-domain ARI", False, "ARI vs truth clusters"),
    ...
    for b, c in zip(BINS, cols):
        ...
        mm = np.nanmedian(np.array(cur, float), 0)
        ax.plot(range(len(SZ)), mm, "o-", color=c, lw=1.3, ms=3, label=f"{b} µm")
        ok = np.isfinite(mm)
        slopes.append(np.polyfit(np.arange(len(SZ))[ok], mm[ok], 1)[0] if ok.sum() > 1 else np.nan)
    ...
    ax.set_title("Hotspot recall by size\n"
                 f"size selectivity {slopes[BINS.index(16)]:+.3f} → {slopes[-1]:+.3f}",
                 fontsize=6.2)
