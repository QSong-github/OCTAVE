# -*- coding: utf-8 -*-
"""CPU 探针：hibou_b / gigapath_flash 走真实的 encoder()+embed_and_barcodes() 跑最小的一片。"""
import sys, time, numpy as np, torch
sys.path.insert(0, "/blue/qsong1/wang.qing/systema4ST/src")
import hest_embed_v2 as H
P = "/blue/qsong1/wang.qing/he2st/HEST/eval/bench_data/CCRCC/patches/INT1.h5"
torch.set_num_threads(4)
for name in ["gigapath_flash", "hibou_b"]:
    t0 = time.time()
    try:
        m, kind = H.encoder(name, "cpu")
        X, bc = H.embed_and_barcodes(m, kind, P, "cpu", bs=32)
        print("  ✓ %-15s X=%s  |mean|=%.3f std=%.3f  nan=%d  kind=%s  %.0fs" % (
            name, X.shape, abs(X.mean()), X.std(), int(np.isnan(X).sum()),
