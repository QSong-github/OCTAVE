...
ENC = sorted({_o.path.basename(f)[len("results/hest_mlp100_ps_") - len("results/"):-5]
              for f in _g.glob("results/hest_mlp100_ps_*.json")} - {"omiclip_raw"})
PS = {}
for e in ENC:
    f = f"results/hest_mlp100_ps_{e}.json"
    if os.path.exists(f):
...
if len(OUT) > 1:
    ...
    json.dump({str(k): OUT[k] for k in OUT}, open("results/k_sensitivity_mlp100.json", "w"), indent=1)
    print("\n已存 results/k_sensitivity_mlp100.json")
