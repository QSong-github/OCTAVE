...
#!/bin/bash
#SBATCH -J emb_${TAG}
#SBATCH --qos=qsong1 --partition=hpg-b200,hpg-rtx6000,hpg-turin --gres=gpu:1 --array=0-$((N-1)) -c 6 --mem=16G -t 24:00:00
...
J3=$(sbatch --parsable --qos=qsong1 -p hpg-default -c 2 --mem=16G -t 1:00:00 -J agg_${TAG} -o logs/agg_${TAG}_%j.out --wrap="source /blue/qsong1/wang.qing/miniconda3/etc/profile.d/conda.sh; conda activate hest; cd /blue/qsong1/wang.qing/systema4ST; python3 ridge_loco.py $(echo $OK | tr ' ' ',') && python3 blk_agg.py | tail -8 && python3 methods_agg2.py | tail -12 && python3 pcount3.py $OK && S4ST_RESULTS=results python3 headline_numbers.py")
