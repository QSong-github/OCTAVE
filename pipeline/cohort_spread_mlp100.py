...
出口带断言：赢的队列数必须与 results/k_sensitivity_mlp100.json 逐个相等。
"""
import glob, json, os
import numpy as np
...
ENC = sorted({_o.path.basename(f)[len("hest_mlp100_ps_"):-5]
              for f in _g.glob(R + "/hest_mlp100_ps_*.json")} - {"omiclip_raw"})
K50 = json.load(open(R + "/k_sensitivity_mlp100.json"))["50"]
OUT, bad = {}, []
for e in ENC:
    ps = R + "/hest_mlp100_ps_%s.json" % e
...
json.dump(OUT, open(R + "/cohort_spread_mlp100.json", "w"), indent=1)
print("%d 个编码器 -> %s/cohort_spread_mlp100.json" % (len(OUT), R))
