...
H2 = "/path/to/project/methods/HGGEP"
...
from hg_fast import build_adj_hypergraph_fast, verify   # 向量化替换，见该文件说明
verify(_HGmod.build_adj_hypergraph)                     # 与作者原版逐位比对，不过就抛错
_HGmod.build_adj_hypergraph = build_adj_hypergraph_fast
...
def main():
    ...
    ap.add_argument("--out", default="results/hggep_hest.json")
    a_ = ap.parse_args()
    ...
    for c in a_.cohorts.split(","):
        ...
        for kf in range(len(glob.glob(os.path.join(B, c, "splits", "test_*.csv")))):
            ...
            model = HGGEP(n_genes=len(genes), n_pos=npos, learning_rate=a_.lr,
