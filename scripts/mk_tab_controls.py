...
for e in encs:
    ...
    rows.append(dict(enc=e, img=np.median(img), coord=np.median(co), rand=np.median(ra), train=np.median(tr), ridge=np.median(rg), dev=float(np.max(np.abs(img - img2))), ind=float(np.median(ind)), coh_ind_gt_ridge=int((cm(ind) > cm(rg)).sum()), ind_over_ridge=float(np.median(cm(ind) / cm(rg))),
                     coh_img_gt_coord=int((cm(img) > cm(co)).sum()), coh_train_gt_ridge=int((cm(tr) > cm(rg)).sum()), coh_coord_gt_ridge=int((cm(co) > cm(rg)).sum()),
                     train_over_ridge=float(np.median(cm(tr) / cm(rg))), coord_over_img=float(np.median(cm(co) / cm(img))), rand_over_img=float(np.median(cm(ra) / cm(img))), coord_over_ridge=float(np.median(cm(co) / cm(rg)))))
...
# ── 表：Xenium 对照与退化 ──
if X and xen.get("n_regions", 0) == 16:
    def f3(t): return f"{t[0]:.3f} & {t[1]:.3f}--{t[2]:.3f}"
    rr = xen["ridge"][0]
    L = [r"\begin{tabular}{lrrr}", r"\toprule", r"Predictor on Xenium ($16$ regions, $8$ specimens) & PCC & Range over specimens & Fraction of ridge \\", r"\midrule",
...
L += [frow("Ridge on frozen features (cohort-selected $\\lambda$)", "ridge"), frow("Image partition, measured means (domain oracle)", "img", None), frow("Coordinate partition, measured means", "coord", "coh_coord_gt_ridge"),
      frow("Random matched-size partition, measured means", "rand"), frow("Image partition, means from training samples", "train", "coh_train_gt_ridge"), frow("Image partition learned on training samples, means from training samples", "ind", "coh_ind_gt_ridge"), r"\bottomrule", r"\end{tabular}"]
