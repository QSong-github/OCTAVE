...
def _sp(n):
    ...
    if "Human_Lung_Cancer_FFPE" in n: return "Lung"
    return n
_NREG = len(_REGS); _NSPEC = len({_sp(n) for n in _REGS})
...
if cp:
    a, b, dp, db = cp[0]; cp_txt = f" The scalar separates some pairs by less than $0.01$. {NAME.get(a,a)} and {NAME.get(b,b)} differ by ${dp:.4f}$ in $\\mathrm{{PCC}}$ and by ${abs(db):.3f}$ in $\\beta_1$. That is a {abs(db)/max(E[a]['bands']['1'],E[b]['bands']['1'])*100:.0f}\\% difference in finest-band fidelity, and the scalar records it as a tie."
sig = float(np.median([E[e]["sigma"] for e in E]))
_bp = f"{R}/beta1_rank_boot.json"; boot_txt = ""
if os.path.exists(_bp):
    _b = json.load(open(_bp)); boot_txt = (f" The ranking is not sampling noise. We resample the ${_NSPEC}$ specimens with replacement ($B={_b['B']}$). The finest-band ranking stays at Spearman ${_b['spearman_boot_vs_full']['med']:.2f}$ [{_b['spearman_boot_vs_full']['lo']:.2f}, {_b['spearman_boot_vs_full']['hi']:.2f}] to the full-data ranking, and the leader stays first in every resample. The direction holds for ${_b['rev_stable95']}$ of the ${_b['n_rev']}$ reversed pairs in at least $95\\%$ of resamples (${_b['rev_stable80']}$ at $80\\%$). {NAME.get('dinov3_vith16','DINOv3-H+')} leads {NAME.get('keep','KEEP')} in $\\beta_1$ by ${-_b['keep_minus_dinov3h']['hi']:.3f}$ to ${-_b['keep_minus_dinov3h']['lo']:.3f}$ across resamples, and the lead never changes sign. Per-encoder $\\beta_1$ intervals have a median width of ${_b['b1_ci_width']['med']:.3f}$.")
...
txt = (f"\\subsection{{{_w(n-1).capitalize()} further encoders under the same readout}}" "\n" r"\label{app:multi}" "\n"
       f"Section~\\ref{{sec:exp:main}} compares one trained model with the domain oracle. Table~\\ref{{tab:multi}} repeats the whole computation for ${n}$ encoders in all. Each uses the same ridge head, the same folds and the same operator. They span {_pstr(min(_pm))} to {_pstr(max(_pm))} parameters and cover general-purpose and pathology pre-training, including distilled students. The scalar compresses them. The ${n}$ scores span ${pcc.min():.3f}$ to ${pcc.max():.3f}$, a range of ${pcc.max()-pcc.min():.2f}$. The finest-band correlations span ${b1.min():.5f}$ to ${b1.max():.5f}$, a factor of ${b1.max()/b1.min():.1f}$. {_lead} The ranking is also scale-dependent. Against the scalar ranking, the finest band reorders ${rk['1']['reversals']}$ of the ${pairs}$ encoder pairs (Spearman ${rk['1']['spearman']:.2f}$), the $t=4$ band ${rk['4']['reversals']}$ (Spearman ${rk['4']['spearman']:.2f}$) and the $t=32$ band ${rk['32']['reversals']}$ (Spearman ${rk['32']['spearman']:.2f}$), so the same leaderboard reads differently at different widths.{cp_txt}{boot_txt} The result of Section~\\ref{{sec:exp:main}} is not a property of one encoder. Every encoder's own domain oracle reaches ${100*rat.min():.0f}$--${100*rat.max():.0f}\\%$ of its score. The finest-band shortfall exceeds the scalar shortfall by a factor of ${gr.min():.1f}$ to ${gr.max():.1f}$ across the ${n}$." "\n"
       r"\begin{table}[h]" "\n" r"\centering\small" "\n" "\\caption{" + _w(n).capitalize() + (r" encoders on the $%d$ Xenium regions under the same protocol." % _NREG) + r" Rows are ordered by the encoder's own score. Medians over specimens of per-specimen medians over regions; $\beta$ at $t=32$ has a median width of $52\,\mu$m. M is the parameter count in millions, $\beta_{32}$ the band at $t=32$, Oracle/model the domain oracle's share of the encoder's own score, written as a fraction here and in per cent in the text, and $\Delta$ ratio the finest-band to scalar shortfall ratio of Section~\ref{sec:exp:main}. The last entry of the right-hand block is the mean over the encoders.}" "\n" r"\label{tab:multi}" "\n" r"\resizebox{\textwidth}{!}{\input{tab_multi}}" "\n" r"\end{table}" "\n")
