...
while true; do
  for J in $(squeue -u wang.qing -h -t PD -o "%i %j" | grep -E " ($MINE)$" | awk '{print $1}' | sed 's/_\[.*//; s/_[0-9]*$//' | sort -u); do
  ...
  R=$(squeue -u wang.qing -h -t R -o "%C" | paste -sd+ | bc 2>/dev/null); R=${R:-0}
  RB=$(squeue -u wang.qing -h -t R -n $BIG -o "%C" | wc -l); RBC=$(squeue -u wang.qing -h -t R -n $BIG -o "%C" | paste -sd+ | bc 2>/dev/null); RBC=${RBC:-0}
  FREE=$((CAP - R))
  if [ $FREE -ge 0 ]; then ALLOW=$((RB + FREE / PER)); else ALLOW=$((RB - ( -FREE + PER - 1) / PER)); fi
  [ $ALLOW -lt 0 ] && ALLOW=0
  PEND=$(squeue -u wang.qing -h -t PD -o "%j" | grep -v -c "^$BIG$"); [ $PEND -gt 0 ] && [ $ALLOW -gt 0 ] && ALLOW=$((ALLOW - 1))
  LINE=""
  for J in $(squeue -u wang.qing -h -n $BIG -o "%i" | grep -o "^[0-9]*_\[" | tr -d "_[" | sort -u); do
    RBJ=$(squeue -u wang.qing -h -t R -n $BIG -o "%i" | grep -c "^${J}_"); T=$((ALLOW - (RB - RBJ))); [ $T -lt 0 ] && T=0
  ...
  n=$(squeue -u wang.qing -h -n $BIG | wc -l); [ "$n" -eq 0 ] && { echo "大阵列结束，调速器退出"; break; }
  sleep 60
done
