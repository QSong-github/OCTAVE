...
python - <<'PY'
import torch, os
src = "/blue/qsong1/wang.qing/systema4ST/stflow_run/weights/ciga/tenpercent_resnet18.ckpt"
dst = "/blue/qsong1/wang.qing/systema4ST/stflow_run/weights/fm_v1/ciga/tenpercent_resnet18.ckpt"
os.makedirs(os.path.dirname(dst), exist_ok=True)
# 官方 GitHub release(已发表论文), 来源可信 → weights_only=False 仅用于这一次转换
ck = torch.load(src, map_location="cpu", weights_only=False)
sd = {k: v for k, v in ck["state_dict"].items() if torch.is_tensor(v)}
torch.save({"state_dict": sd}, dst)
back = torch.load(dst, map_location="cpu")          # 默认 weights_only=True, 验证可读
print(f"  ✅ 转换完成 {len(sd)} 个张量; 安全模式回读 OK; 键样例 {list(sd)[:3]}")
PY
