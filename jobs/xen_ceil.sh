#!/bin/bash
#SBATCH -J xenceil
#SBATCH --qos=YOUR_QOS --partition=YOUR_CPU_PARTITION --array=0-15 -c 8 --mem=48G -t 4:00:00
...
R=${REG[$SLURM_ARRAY_TASK_ID]}; [ -s results/xen_band_ceiling/${R}.json ] && exit 0
python -u xen_band_ceiling.py --name $R
