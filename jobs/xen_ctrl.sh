#!/bin/bash
#SBATCH -J xenctrl
#SBATCH --qos=YOUR_QOS --partition=YOUR_CPU_PARTITION --array=0-15 -c 8 --mem=48G -t 8:00:00
...
R=${REG[$SLURM_ARRAY_TASK_ID]}; [ -s results/xen_controls/${R}.json ] && exit 0
python -u xen_controls.py --name $R
