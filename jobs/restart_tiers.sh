...
echo "取消待运行部分后仍在跑: $(squeue -u $USER -h -t R -n xenmulti | wc -l)"
...
sleep 5; tail -1 logs/cpu_governor.log; squeue -u $USER -h -n xenmulti -o "%i %t %r" | sed 's/_[0-9]* / /' | awk '{print $2, $3}' | sort | uniq -c | tr "\n" ";"; echo
