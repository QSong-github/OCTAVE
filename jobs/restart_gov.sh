...
scancel -t PENDING -n xenmulti -u $USER; sleep 2
...
J=$(sbatch --parsable jobs/xen_multi_an_all2.sh); echo "新阵列 xenmulti=$J"
nohup bash jobs/cpu_governor.sh >> logs/cpu_governor.log 2>&1 &
sleep 3; tail -2 logs/cpu_governor.log; squeue -u $USER -h -n xenmulti -o "%i %t %C" | sed 's/_[0-9]* / /' | awk '{print $2, $3}' | sort | uniq -c | tr "\n" ";"; echo
