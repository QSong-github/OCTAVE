#!/bin/bash
#SBATCH -J xenind
#SBATCH --qos=YOUR_QOS --partition=YOUR_CPU_PARTITION --array=0-15 -c 8 --mem=64G -t 8:00:00
...
R=${REG[$SLURM_ARRAY_TASK_ID]}; [ -s results/xen_inductive/${R}.json ] && exit 0
python -u xen_inductive.py --name $R
