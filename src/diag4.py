...
S = {"母项目": openslide.OpenSlide(A), "10x无损": openslide.OpenSlide(L), "10x-JPEG": openslide.OpenSlide(J)}
...
b = np.array(acc["母项目"]); o = np.array(acc["10x无损"])
print(f"\n母项目 / 10x 原图 的比值：梯度 {b[:,0].mean()/o[:,0].mean():.4f}  "
      f"Laplacian {b[:,1].mean()/o[:,1].mean():.4f}  高频占比 {b[:,2].mean()/o[:,2].mean():.4f}")
print("⇒ " + ("母项目的图更模糊 —— 其等价 σ 含图像预处理造成的高估"
              if b[:,1].mean() < o[:,1].mean()*0.95 else
              "母项目的图更锐 —— 可能做过锐化/去噪" if b[:,1].mean() > o[:,1].mean()*1.05 else
