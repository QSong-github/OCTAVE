...
print(f"  ⇒ {'上游项目那张图的内容本身与 10x 原图不同（处理过，来源不可考）' if np.median(maes)>1 else '像素一致，差异另有来源'}")
...
for c, nm in enumerate("RGB"):
    d = ta[...,c]-tl[...,c]
    print(f"  {nm}: 上游项目均值={ta[...,c].mean():7.2f}  10x均值={tl[...,c].mean():7.2f}  差均值={d.mean():+6.2f} 差std={d.std():5.2f}")
