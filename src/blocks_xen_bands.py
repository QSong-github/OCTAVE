...
OUTD = "/blue/qsong1/wang.qing/systema4ST/results/blocks_xen_bands"
...
def main():
    ...
    # 全带：B_i = S_{i-1} - S_i，逐档在 ok bin 上评估（W 为全图算子，物理一致）
    def band(M):
    ...
    def all_bands(M):
        ...
        for cp in cps:
            while t < cp:
                prev = W @ prev; t += 1
            out[cp] = np.asarray(low - prev, np.float32)
            low = prev.copy()
        return out
    BT = band(Y)
    ABT = all_bands(Y)
    ...
    for nm, M in preds.items():
        ...
        AB = all_bands(Mf)
        bands = {str(cp): float(np.nanmean(per_gene_pcc(AB[cp][ok], ABT[cp][ok])))
                 for cp in cps}
    ...
    R["truth"] = {"fine_var_share": float(np.mean(BT[ok].var(0) / (Y[ok].var(0) + 1e-12))),
                  "band_var": {str(cp): float(np.mean(ABT[cp][ok].var(0)
                                                      / (Y[ok].var(0) + 1e-12)))
                               for cp in cps}}
