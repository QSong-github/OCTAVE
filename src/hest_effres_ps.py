...
def main():
        ...
        for k in range(len(glob.glob(os.path.join(B, c, "splits", "test_*.csv")))):
            ...
            if args.head == "mlp":
                ...
                _m = MLPRegressor(hidden_layer_sizes=(512,), activation="relu", solver="adam",
                                  alpha=args.mlp_alpha, batch_size=256, learning_rate_init=1e-3,
            ...
            for s in te:
                ...
                for cp in cps:
                    ...
                    ps_sigma.setdefault(s, {})[str(cp)] = float(sig[cp])
    ...
    res = {"per_sample_band_pcc": ps_band, "per_sample_sigma_um": ps_sigma,
           ...
           "encoder": args.encoder, "pcc_check": check, "eq_sigma_ladder": eq_stats,
