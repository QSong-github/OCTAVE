...
for s in SLIDES:
    p = f"/path/to/upstream_align/st_bench/data/{s}/adata_16um.h5ad"
...
for p in ["/path/to/upstream_stbench/cosmx_hs_nsclc.h5ad"]:
    desc(p, "CosMx NSCLC")
...
for p in sorted(glob.glob("/path/to/upstream/**/MERFISH*16um*.h5ad", recursive=True))[:2]:
    desc(p, "MERFISH 16µm")
print("\n"+"="*72); print("④ 该目录下所有 H&E / 图像文件"); print("="*72)
n = 0
for root in ["/path/to/upstream"]:
